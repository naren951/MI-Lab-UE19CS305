...
class AdaBoost:
    ...
    def compute_alpha(self, error):
        ...
        if error == 0:
            error = eps
        # else:
        # print((1/2)*np.log((1-error)/error))
    ...
    def update_weights(self, y, y_pred, sample_weights, alpha):
        ...
        performance = self.compute_alpha(error)
        new_weights = []
        for i in range(len(sample_weights)):
            if(y[i]==y_pred[i]):
                wts=(sample_weights[i])*(np.exp(-performance))
            else:
                wts=(sample_weights[i])*(np.exp(performance))
            new_weights.append(wts)
    ...
    def predict(self, X):
        ...
        """
        # TODO
        pred = np.array([self.stumps[stump].predict(X) for stump in range(self.n_stumps)])
